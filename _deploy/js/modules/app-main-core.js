  ...
  const requiredFields = ['id', 'outlet_code', 'outlet_name', 'address', 'phone', 'sale_name', 'area', 'items', 'total_amount', 'created_at', 'spo_number'];
    ...
    async function initializeApp() {
      ...
      const resolvedDataHandler = (() => {
        try {
          if (window.__qcagDataHandler) return window.__qcagDataHandler;
        } catch (e) {}
        try {
          if (typeof dataHandler !== 'undefined') return dataHandler;
        } catch (e) {}
        return null;
      })();

      if (!window.dataSdk || typeof window.dataSdk.init !== 'function') {
        console.error('[QCAG] dataSdk is unavailable. Cannot initialize backend data flow.');
        return;
      }
      if (!resolvedDataHandler) {
        console.error('[QCAG] dataHandler is unavailable. Cannot process loaded data.');
        return;
      }

      const dataSdkResult = await window.dataSdk.init(resolvedDataHandler);
